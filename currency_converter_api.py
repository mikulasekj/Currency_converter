...
def abort_if_input_currency_not_exist():
    """handles the WrongInputCurrencyError exception in response"""
    abort(404, message="The input currency is not supported")

def abort_if_ouput_currency_not_exist():
    """handles the WrongOutputCurrencyError exception in response"""
...
class Inputs(Resource):
    def get(self):
        ...
        try:
            response = Response(
                response=convertor_obj.to_convert(input_currency, amount, output_currency))
